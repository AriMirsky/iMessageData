...
def read_oldest_messages(db_location, name, n=None, self_number='Me', human_readable_date=True):
    conn = sqlite3.connect(db_location)
    cursor = conn.cursor()
    ...
    FROM message
    LEFT JOIN handle ON message.handle_id = handle.ROWID
    WHERE (handle.id IN ({possible_numbers}) or handle.id IN ({possible_emails}))
